...
_fnc_setState = switch _state do {
	...
	case "AUTOASSIGNED": {
		{if (isnull currenttask _x) then {_stateNew = "ASSIGNED"; _x setcurrenttask _task;} else {_stateNew = "CREATED"; _task settaskstate _stateNew;};}
	};
	default {
	...
	if (!(_x in _unitsUsed) && !isnull _x) then {
		...
		if (_x == player && _showNotification) then {
			...
			if (_stateNew != _stateOld) then {
				["task" + _stateNew,_desc] call bis_fnc_shownotification;
			};
		};
