...
_createdUnits = [];
_turrets = ([_class] call BL_fnc_vehicleWeapons);

// If a list of units to use wasn't provided add riflemen for driver/turret(s)
if ( count _units == 0 ) then {
	for "_i" from 0 to (1 + (count _turrets -1)) do {
...
{
	(_createdUnits select (_forEachIndex + 1)) moveInTurret [_veh, _x select 1];
} forEach _turrets;

for "_i" from (1 + (count _turrets -1)) to (count _createdUnits) - 1 do {
