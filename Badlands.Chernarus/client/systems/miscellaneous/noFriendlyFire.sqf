[] spawn {
	_noFriendlyFire = {
		...
		if ( alive _player && (effectiveCommander _player) != (effectiveCommander _attacker) && (_attacker in (units group player) || ( playerSide in [east,west] && side player == side _attacker )) ) then {
