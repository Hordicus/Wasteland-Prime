...
_group    = [_this, 0, false, [grpNull, false]] call BIS_fnc_param;
_loc      = [_this, 1, [0,0,0], [[]], [2, 3]] call BIS_fnc_param;
_units    = [_this, 2, [], [[]]] call BIS_fnc_param;
_spread   = [_this, 3, 100, [0]] call BIS_fnc_param;
_maxDist  = [_this, 4, 300, [0]] call BIS_fnc_param;
_aware    = [_this, 5, true, [true, []]] call BIS_fnc_param;
_bounty   = [_this, 6, BL_aiBountyAmount, [0]] call BIS_fnc_param;
_nice     = [_this, 7, true, [true]] call BIS_fnc_param;
_cbParams = [_this, count _this-2, [], [[]]] call BIS_fnc_param;
_cb       = [_this, count _this-1, {}, [{}]] call BIS_fnc_param;
...
if ( _maxDist > 0 ) then {
	[_maxDist, _loc, _group] spawn {
		...
		while { {alive _x} count (units _group) > 0 } do {
			...
			{
				diag_log [_x, _x distance _loc, _x distance _loc > _maxDist];
