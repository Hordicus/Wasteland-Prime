...
_unitsUsed = [];
{
	if (!(_x in _unitsUsed) && !isnull _x) then {
		...
		_stateOld = if (isnil {_task}) then {
			...
			_taskParentVar = (_params select 1) call BL_fnc_taskVar;
			...
			_xTasks = _x getvariable ["BL_fnc_setTaskLocal_tasks",[]];
			_xTasks set [count _xTasks,_taskID];
			_x setvariable ["BL_fnc_setTaskLocal_tasks",_xTasks];
			""
		} else {
			taskstate _task
		};
		...
		if (_stateOld == "ASSIGNED" && toupper _state in ["SUCCEEDED","FAILED","CANCELED"]) then {
			...
			} foreach (_x getvariable ["BL_fnc_setTaskLocal_tasks",[]]);
			if (_nextTask != "") then {
				[_nextTask,nil,nil,nil,true,nil,false] call BL_fnc_setTask;
			};
		};
