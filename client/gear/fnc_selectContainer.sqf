...
((findDisplay GEAR_dialog_idc) displayCtrl (_index call GEAR_loadoutIndexToIDC)-1) ctrlSetBackgroundColor [1,1,1,0.5];

if ( !isNil "_container_type" ) then {
	_items = GEAR_activeLoadout select (_index+1);
	{
		_name = _x call GEAR_ItemName;
		_img = _x call GEAR_ItemImg;
		_price = _x call GEAR_itemPrice;
